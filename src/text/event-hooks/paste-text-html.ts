...
function pasteTextHtml(editor: Editor, pasteEvents: Function[]) {
    function fn(e: Event) {
        ...
        if (urlRegex.test(pasteText) && pasteFilterStyle) {
            ...
            const urlArray = pasteText.match(urlRegex)
            const resultText = pasteText.replace(urlRegex, '<a href="" target="_blank"></a>')
            ...
            // 修改a 标签的href 和 链接文案，这样就避免了使用innerHTML时把特殊符号转义
            const linkEle = fragment.querySelectorAll('a')
            linkEle.forEach((ele, index) => {
                ele.innerText = urlArray![index]
                ele.href = urlArray![index]
            })
